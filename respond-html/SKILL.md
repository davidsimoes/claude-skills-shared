---
name: respond-html
description: "Render a structured HTML artifact instead of a chat reply — for plans, proposals, comparisons, audits, recommendations, mockups, and process/data visualizations. Single self-contained file saved to ~/dev/personal/html-answers/, opened in browser, with inline ✅/💬/❌ reactions + comment box on every section, decision-block, and callout. Live stats in sticky toolbar. One button copies all feedback as markdown for David to paste back to chat. Use proactively when the user says 'show me a plan for X', 'compare approaches to X', 'mock up X', 'audit X', 'visualize this', 'html answer', 'html response', 'canvas this', 'structure this as html', 'show me visually', 'render this', '/respond-html', or any time the response is plan-shaped (3+ options with tradeoffs), proposal-shaped (recommendation with rationale), comparison-shaped (matrix or matrix-able), or audit-shaped (findings + severity + recommendations) — even if the user doesn't explicitly ask for HTML. The skill is an orchestrator: it routes data analysis to /dataviz data mode, process/workflow/architecture to /dataviz process mode, UI/component design to /frontend-design, and uses its own reading-optimized template for everything else (plans, proposals, audits, comparisons, structured analysis). Always finishes with /chrome-validate before declaring ready."
user-invocable: true
...
When a chat reply would be a wall of prose with implicit structure (sections, comparisons, decisions, tradeoffs), the artifact wins. David scans, reacts inline (✅/💬/❌ + comment per section/decision-block/callout), hits 📋 Copy feedback, pastes the markdown back to chat. The skill makes that the default path, not a special occasion.
...
4. **Opens** the file in David's browser via `open`.
...
- **Output path divergence**: /dataviz defaults to `data/{slug}_{hash6}_report.html` in the current working directory; /frontend-design typically produces code in chat or scaffolds a framework project, not a single HTML file. Phase 3 (chrome-validate) and Phase 4 (`open`) below assume `~/dev/personal/html-answers/<slug>/index.html` (or the project-aware path resolved in Phase 2) exists — when delegating, you must either:
...
**Output path** (project-aware in v0.3):

1. **Look at `pwd`**. If it's inside `~/dev/SGA/<proj>/`, `~/dev/personal/<proj>/`, or `~/dev/brain/projects/<area>/`, write to `<that-root>/.html-answers/<slug>/index.html`.
2. **Otherwise scan the content** for project-name keywords (Reservio, Mandimu, Orkla, BAPA, Soundsgood, sga-redesign25-theme, slack-resolve, respond-html, kalla, etc. — match against `ls ~/dev/SGA/` and `ls ~/dev/brain/projects/`). If a clear match, use that project's `.html-answers/`.
3. **Fallback** for cross-cutting / skill-meta / "no specific project" content: `~/dev/personal/html-answers/<slug>/index.html`.

`<slug>` is a 3-5 word kebab-case summary of the topic (e.g., `compare-deploy-targets`, `audit-missive-triage`, `q3-roadmap-plan`).

State the chosen path inline before rendering: *"Detected Reservio project — saving to `~/dev/SGA/reservio/.html-answers/migration-vercel-plan/`."* If you're unsure (no `pwd` match, no clean keyword hit), surface a single AskUserQuestion with 2-3 plausible roots before rendering.

The path determines where TTL cleanup later sweeps from (see Phase 5 + `/close` integration in v0.3).
...
3. **Body sections** — the actual content. Use callouts (`decision`, `tradeoff`, `risk`, `recommendation`) and decision-blocks (question + options + recommendation) where they earn their keep. Each section, decision-block, and callout becomes a **reactable unit** in the feedback model — David can ✅/💬/❌ each one independently.
4. **Open questions / next steps** — what's unresolved, what David needs to decide.
...
- One button copies grouped-by-section markdown to clipboard for David to paste back.
...
Surface the GATE/STATUS/EVIDENCE block to David. Do not say "ready" until every gate is PASS. On FAIL:
- Show David the failing gate + the evidence
...
Plus a one-line summary of what the artifact contains (so David knows what to look at without opening it first).
...
- **Auto-fixing chrome-validate failures without showing David first.** Failures often surface real issues he wants to weigh in on (a missing section, wrong content, broken link to a doc that doesn't exist yet). Show, then fix.
- **Deploying to Vercel.** This skill is local-only. If David wants to share, he'll say so — at which point use `/indicative-offer` or push manually. Don't deploy by default.
- **Opening multiple files.** Always `open` exactly the `index.html`. The browser-spam pattern is annoying and breaks David's flow.
...
Documented gaps rather than silent failures — David should know what's unsupported:
...
- **Reactions persist locally in browser localStorage** keyed by pathname. They survive reload but don't migrate across origins (file:// vs localhost), and don't survive across browsers/devices. There's no server-side persistence — David's expected to react in a single session, copy as markdown, paste back to chat.
...
David's been hand-crafting HTML structured responses for plans, approaches, mockups, and dataviz. Recurring pattern: open chat → reach for HTML → realize the chat reply is the wrong shape. This skill makes the artifact the default — with inline ✅/💬/❌ reactions per reactable unit so David can mark up the artifact and redirect — and reuses existing skills (`/dataviz`, `/frontend-design`, `/chrome-validate`) instead of duplicating their work.
